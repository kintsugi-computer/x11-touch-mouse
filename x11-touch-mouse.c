...
#define UNKWN_MOUSE_BTN 0
#define LEFT_MOUSE_BTN 1
#define RIGHT_MOUSE_BTN 2
...
static int process_events(int fd)
{
    ...
    int tracking_id = 0;
    struct timeval event_time, touch_diff;
    // 100 milliseconds window to allow registering a second
    // touch event as a right mousebutton down event
    struct timeval touch_window = {
        .tv_sec=0,
        .tv_usec=100000
    };
    struct timeval first_touch = {
        .tv_sec=0,
        .tv_usec=0
    };
    ...
    while (!stop) {
        ...
        for (i = 0; i < rd / sizeof(struct input_event); i++) {
            ...
            // if this is a BTN_TOUCH event, set button_down or button_up based on the value
            ...
            // if this is a tracking id event, set the tracking id
            if ( type == EV_ABS && code == ABS_MT_TRACKING_ID ) {
                tracking_id = ev[i].value;
            }

            // if this is a coordinate event (codes 0, 1, 53, 54) and tracking id == 0 then 
            ...
            if ( type == EV_ABS && code == ABS_MT_POSITION_X && tracking_id == 0 ) {
                x = ev[i].value;
            }
            if ( type == EV_ABS && code == ABS_MT_POSITION_Y && tracking_id == 0 ) {
                y = ev[i].value;
            }
            ...
            if ( type == EV_SYN && (code == SYN_MT_REPORT || code == SYN_REPORT) ) {
                ...
                if ( mouse_button_down && active_mouse_button == UNKWN_MOUSE_BTN ) {
                    ...
                    event_time.tv_usec = ev[i].input_event_usec;

                    if ( first_touch.tv_sec == 0 ) {
                        // The first SYN or SYM_MT_REPORT will have tracking_id == 0
                        first_touch.tv_sec = event_time.tv_sec;
                        first_touch.tv_usec = event_time.tv_usec;
                    } else {
                        timersub(&event_time, &first_touch, &touch_diff);
                        within_window = timercmp(&touch_diff, &touch_window, <);
...
static int do_capture(const char *device, int grab_flag)
{
    ...
    if (test_grab(fd, grab_flag))
    {
        ...
        printf("  No events are available to x11-touch-mouse while the\n"
               "  other grab is active.\n");
        printf("  In most cases, this is caused by an X driver,\n"
               "  try VT-switching and re-run x11-touch-mouse again.\n");
